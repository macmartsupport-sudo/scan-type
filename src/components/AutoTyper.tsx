...
import { Play, Pause, RotateCcw, Volume2, VolumeX, Keyboard, Sparkles, Sliders, CheckCircle, Zap, ShieldAlert, AlertTriangle, FileText, Info, Download, ChevronDown, FileJson } from "lucide-react";
...
export default function AutoTyper({ document: doc, config, onChangeConfig }: AutoTyperProps) {
  ...
  const handleDownloadTxt = () => {
    ...
    const file = new Blob([doc.rawText], { type: "text/plain;charset=utf-8" });
    element.href = URL.createObjectURL(file);
    const name = doc.suggestedFilename || "scanned_document";
    element.download = name.endsWith(".txt") ? name : `${name}.txt`;
  ...
  const handleDownloadJson = () => {
    const element = window.document.createElement("a");
    const file = new Blob([JSON.stringify(doc, null, 2)], { type: "application/json;charset=utf-8" });
    element.href = URL.createObjectURL(file);
    const name = doc.suggestedFilename || "scanned_document";
    const cleanName = name.replace(/\.[^/.]+$/, "");
    element.download = `${cleanName}.json`;
    window.document.body.appendChild(element);
    element.click();
    window.document.body.removeChild(element);
    setShowDownloadMenu(false);
  ...
  useEffect(() => {
    ...
    else if (config.speed === "custom") {
      delay = Math.round(60000 / (customWpm * 5));
  ...
  }, [isPlaying, currentIndex, fullText, config, customWpm]);
  ...
  const handleSpeedChange = (speed: TypingSpeed) => {
    ...
    if (speed === "slow") setCustomWpm(90);
    else if (speed === "medium") setCustomWpm(200);
    else if (speed === "fast") setCustomWpm(480);
    else if (speed === "human") setCustomWpm(110);
    else if (speed === "instant") setCustomWpm(1000);
  };

  const handleCustomWpmChange = (wpm: number) => {
    setCustomWpm(wpm);
    onChangeConfig({ ...config, speed: "custom" });
  };
      ...
      <div className="lg:col-span-5 space-y-6">
        ...
        <div className="bg-white rounded-[32px] p-6 border border-indigo-100 shadow-sm space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-1.5 p-1 bg-slate-50 rounded-xl" id="speed-selector">
              {(["slow", "medium", "fast", "human", "instant", "custom"] as TypingSpeed[]).map((sp) => (
                <button
                  key={sp}
                  type="button"
          ...
          {/* Precision Pace WPM Slider */}
          <div className="space-y-2 pt-2 border-t border-indigo-50/50">
            <div className="flex justify-between items-center">
              <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                Precision Pace (WPM)
              </span>
              <span className="text-xs font-extrabold text-indigo-600 bg-indigo-50 px-2.5 py-0.5 rounded-lg font-mono">
            ...
            <input
              ...
              max="1000"
              step="10"
              value={config.speed === "instant" ? 1000 : config.speed === "human" ? 110 : customWpm}
              onChange={(e) => handleCustomWpmChange(Number(e.target.value))}
            ...
            <p className="text-[10px] text-slate-400">
              Drag the slider to adjust typing speed dynamically during live playback.
            </p>
          </div>
          ...
          <div className="bg-slate-50 p-4 rounded-2xl flex justify-between items-center text-xs">
            ...
            <div className="flex items-center gap-1.5 font-bold text-slate-800">
              ...
              <span>
                {config.speed === "instant" 
                  ? "∞ (Instantaneous)" 
                  : `${wpmStats || (config.speed === "custom" ? customWpm : config.speed === "slow" ? 90 : config.speed === "medium" ? 200 : config.speed === "fast" ? 480 : config.speed === "human" ? 110 : 240)} WPM`}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="flex gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 text-xs font-bold text-slate-700 px-3 py-2 bg-white rounded-xl border border-slate-200 shadow-sm hover:bg-slate-50 cursor-pointer"
              ...
              {showDownloadMenu && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white border border-slate-100 rounded-2xl shadow-xl z-50 py-1.5 overflow-hidden origin-top-right animate-in fade-in slide-in-from-top-2 duration-150">
                    <button
                      onClick={handleDownloadTxt}
                      className="w-full text-left px-4 py-2.5 text-xs text-slate-700 hover:bg-indigo-50/50 hover:text-indigo-600 flex items-center gap-2 transition-colors cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className="w-full text-left px-4 py-2.5 text-xs text-slate-700 hover:bg-indigo-50/50 hover:text-indigo-600 flex items-center gap-2 transition-colors cursor-pointer"
                    >
                      <FileJson className="w-3.5 h-3.5 text-slate-400 group-hover:text-indigo-500" />
